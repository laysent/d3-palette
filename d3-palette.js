      ...
      var palette = function(svg) {
          ...
          if (Rx && Rx.Observable && variable.dom) {
              var MouseDowns = Rx.Observable.fromEvent(variable.dom, 'mousedown'),
                  ...
                  MouseDrags.map(function(dragPoint) {
                      return dragPoint == null ? null : angle(dragPoint.x, dragPoint.y);
                  }).distinctUntilChanged().forEach(function(endAngle) {
                      if (endAngle == null) { 
                        return;
                      }
                      if (flag) {
                          selectedRange = [(Math.max(startAngle, endAngle) - Math.PI * 2), Math.min(startAngle, endAngle)];
                      } else {
                          selectedRange = [Math.min(startAngle, endAngle), Math.max(startAngle, endAngle)];
                      }
